...
15. **No silent click (no-silent-click policy).** Every user-initiated UI action MUST produce a visible signal — success OR failure — within ~3-4 seconds. The signal can be a success state, a loading state, or a failure banner naming what failed, the most likely cause, and one concrete next step. A click that does nothing is a bug regardless of whether the underlying code "threw." Defend in BOTH directions: install an `onError` handler that converts thrown errors into a visible banner AND install a watcher that observes the expected post-action state and surfaces a banner if it doesn't arrive within the deadline. Never trust third-party libraries (wallet adapters, payment SDKs, OAuth flows, browser extension bridges) to throw — they have silent-return paths. Watching outcomes is OUR job. See `BUG_PREVENTION.md` rule I1 for the cross-project version; caught in `app/src/components/WalletProvider.tsx` on 2026-05-22 when clicking Phantom in the wallet modal returned silently with no visible feedback.
